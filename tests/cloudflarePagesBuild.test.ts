...
describe('Cloudflare Pages build contract', () => {
  ...
  it('keeps Pages and worker Cloudflare configs aligned with the required build flow', () => {
    ...
    expect(workflowConfig).toContain('wrangler pages deploy .pages-deploy --project-name evorupa')
    expect(workflowConfig).toContain('wrangler pages deploy .pages-deploy --project-name gderupa')
    expect(workflowConfig).not.toContain('wrangler pages deploy .pages-deploy --config')
